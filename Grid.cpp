...
std::pair<std::pair<int, int>, std::pair<int, int>>
	Grid::cohenSutherland(const std::pair<int, int> &p1, const std::pair<int, int> &p2, int xMin, int xMax, int yMin, int yMax){
		/**
		 * ALGORITMO
		 * 
		...
		*/
		...
		std::string c1Bin = Grid::getCodigoCS(p1, xMin, xMax, yMin, yMax),
					c2Bin = Grid::getCodigoCS(p2, xMin, xMax, yMin, yMax);
		...
		std::cout << "P2: " << p2.first << ',' << p2.second << std::endl << std::endl;
		...
		if(totalDentro){
			...
			std::pair <unsigned int, unsigned int> infoDiffBit = Grid::getDiffBitCS(c1Bin, c2Bin, xMin, xMax, yMin, yMax);
			...
			std::pair<std::pair<int, int>, std::pair<int, int>> 
				pontosAtualizados = Grid::getPontoInterseccaoCS(infoDiffBit, p1Info, p2Info, xMin, xMax, yMin, yMax);

			std::pair<int, int> novoP1 = pontosAtualizados.first,novoP2 = pontosAtualizados.second;

			return Grid::cohenSutherland(novoP1,novoP2,xMin,xMax,yMin,yMax);
		}
	
}

std::string Grid::getCodigoCS(const std::pair<int, int> &p, int xMin, int xMax, int yMin, int yMax){
	char codigo[4];
...
std::pair < unsigned int, unsigned int> Grid::getDiffBitCS(const std::string &c1Bin, const std::string &c2Bin, int xMin, int xMax, int yMin, int yMax)
{
...
Grid::getPontoInterseccaoCS(
	...
	// const std::pair<int, int> &pontoEscolhido = c1Dec ? p1 : p2,
	// 						  &outroPonto = (pontoEscolhido == p1)? p2 : p1;
	...
	switch(diffBit){
		case 0: //CIMA
			x = xPontoEscolhido - (yPontoEscolhido - yMax) / m;
			y = yMax;
			break;
		case 1: //BAIXO
			x = xPontoEscolhido - (yPontoEscolhido - yMin) / m;
			y = yMin;
			break;
		case 2: //DIREITA
			x = xMax;
			y = yPontoEscolhido - (xPontoEscolhido - xMax) * m;
			break;
		case 3: //ESQUERDA
			x = xMin;
			y = yPontoEscolhido - (xPontoEscolhido - xMin) * m;
			break;
	}
